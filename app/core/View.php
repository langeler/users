...
class View {
	...
	static function includeFiles($file) {
		...
		foreach ($matches as $value) {
			$code = str_replace($value[0], self::includeFiles(VIEWS_DIR . DIRECTORY_SEPARATOR . $value[2]), $code);
		}
	...
	static function compileStatic($code) {
		return preg_replace('/{% ?static ?\'?(.*?)\'? ?%}/i', '<?php echo "' . App::root_url() . '/assets' . DIRECTORY_SEPARATOR . '$1" ?>', $code);
	}

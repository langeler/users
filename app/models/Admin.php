...
class Admin
{
	...
	function update_account(
		...
		$password =
			$account["password"] != $password
				...
				: $account["password"];
		$this->db->query(
			"UPDATE accounts SET username = ?, password = ?, email = ?, activation_code = ?, rememberme = ?, role = ? WHERE id = ?",
			$username,
			$password,
			$email,
			$activation_code,
	...
	static function format_var_html($key, $value)
	{
		...
		$type = in_array(strtolower($value), ["true", "false"])
			? "checkbox"
			: $type;
		$checked = strtolower($value) == "true" ? " checked" : "";
		$html .=
			'<label for="' . $key . '">' . self::format_key($key) . "</label>";
		if ($type == "checkbox") {

...
class User
{
	...
	function authenticate($username, $password, $remember, $token)
	{
		...
		if ($account) {
			if (password_verify($password, $account["password"])) {
				if (
					account_activation &&
					$account["activation_code"] != "activated"
				) {
					return 'Please activate your account to login, click <a href="' .
						App::root_url() .
						'/resendactivation">here</a> to resend the activation email!';
				} else {
					...
					if ($remember) {
						$cookiehash = !empty($account["rememberme"])
							? $account["rememberme"]
							: password_hash(
								$account["id"] .
									$account["username"] .
				...
				if (brute_force_protection) {
					...
					return "Incorrect username and/or password, you have " .
						$login_attempts["attempts_left"] .
						" attempts remaining!";
				}
	...
	function register($username, $password, $cpassword, $email)
	{
		...
		$account = $this->db
			->query(
				"SELECT id, password FROM accounts WHERE username = ? OR email = ?",
				$username,
				$email
			)
			->fetchArray();
		if ($account) {
			...
			$this->db->query(
				"INSERT INTO accounts (username, password, email, activation_code) VALUES (?, ?, ?, ?)",
				$username,
				$password,
				$email,
				$uniqid
	...
	function is_loggedin()
	{
		if (
			...
			!$this->controller->session("loggedin")
		) {
			$account = $this->db
				->query(
		...
		} elseif (!$this->controller->session("loggedin")) {
			return false;
		}
		return true;
	}
	...
	function send_activation_email($email, $code)
	{
		...
		$email_template = str_replace(
			"%link%",
			$activate_link,
			file_get_contents("app/views/activation-email-template.html")
		);
	...
	function update_info($username, $password, $cpassword, $email)
	{
		...
		$account = $this->db
			->query(
				"SELECT * FROM accounts WHERE (username = ? OR email = ?) AND username != ? AND email != ?",
				$username,
				$email,
				$this->info("username"),
				$this->info("email")
		...
		$uniqid =
			account_activation && $this->info("email") != $email
				? uniqid()
				: $this->info("activation_code");
		$password = !empty($password)
			? password_hash($password, PASSWORD_DEFAULT)
			: $this->info("password");
		$this->db->query(
			"UPDATE accounts SET username = ?, password = ?, email = ?, activation_code = ? WHERE id = ?",
			$username,
			$password,
			$email,
			$uniqid,
	...
	function forgot_password($email)
	{
		...
		if ($account) {
			$uniqid = uniqid();
			$this->db->query(
				"UPDATE accounts SET reset = ? WHERE email = ?",
				$uniqid,
				$email
			);
			...
			$headers =
				...
				"\r\n";
			$reset_link =
				App::root_url() . "/resetpassword/" . $email . "/" . $uniqid;
			$message =
				'<p>Please click the following link to reset your password: <a href="' .
				$reset_link .
				'">' .
				$reset_link .
				"</a></p>";
	...
	function reset_password($email, $code, $npass, $cpass)
	{
		$account = $this->db
			->query(
				"SELECT * FROM accounts WHERE email = ? AND reset = ?",
				$email,
				$code
			)
			->fetchArray();
		if ($account) {
			if (isset($npass, $cpass)) {
				if (strlen($npass) > 20 || strlen($npass) < 5) {
					...
					$this->db->query(
						'UPDATE accounts SET password = ?, reset = "" WHERE email = ?',
						$password,
						$email
					);
					return 'Password has been reset! You can now <a href="' .
						App::root_url() .
						'/">login</a>!';
				}
			}
	...
	function resend_activation($email)
	{
		$account = $this->db
			->query(
				'SELECT * FROM accounts WHERE email = ? AND activation_code != "" AND activation_code != "activated"',
				$email
			)
			->fetchArray();
		if ($account) {
	...
	function login_attempts($update = true)
	{
		...
		if ($update) {
			$this->db->query(
				"INSERT INTO login_attempts (ip_address, `date`) VALUES (?,?) ON DUPLICATE KEY UPDATE attempts_left = attempts_left - 1, `date` = VALUES(`date`)",
				$ip,
				$now
			);
		}
		...
		if ($login_attempts) {
			$expire = date(
				"Y-m-d H:i:s",
				strtotime("+1 day", strtotime($login_attempts["date"]))
			);
			if ($now > $expire) {
				$this->db->query(
